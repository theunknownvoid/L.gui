...
StatsHealthBackground.Name = "lancelottム"
...
StatsHealthBar.Name = "lancelottム"
...
local Actyrn = UiLib:CreateWindow("lancelott.guiム | XD SKID!! | ggz!", Vector2.new(500, 600), Enum.KeyCode.RightShift)
...
local TrashTalkTog = UtilitiesSec:AddToggle("Trash Talk", false, function(Value)
	if Value then
		local TrashTalkWords = {"azure skidded by lancelott★", "BASURA", "How to aim pls help", "wow my little brother was playing and beat you :rofl:", "Mobile player beat u Lol XD", "420 ping and u got SLAMMED", "ur bad", "seed", "im not locking ur just bad", "clown", "sonned", "LOLL UR BAD", "dont even try.. ur not enough for the alpha", "ez", "gg = get good", "my grandmas better than u :skull:", "hop off kid", "bro cannot aim", "u got absolutely DOGGED on", "i run this server son", "what is bro doing :skull:", "no way u lost that"}
		...
		if CamBindEnabled then
			...
			if OnScreen then
				...
				if CameraAimbot.Notify then
					StarterGui:SetCore("SendNotification", {
						Title = "ム [lancelottxd]", 
						Text = "Locked: " .. CamlockPlr.DisplayName, 
						Duration = 2.5
					})
				end
			end
